...
1. `startRecording()` requests microphone access and starts `MediaRecorder`. Android supplies the four-minute timeslice directly; non-Android recorders use the external stop/restart rotation interval.
2. Audio blobs are serialized before entering the ordered upload queue. Android timeslice events are regular chunks, while an intentional inactive stop event is the one final chunk.
...
6. Stop from `paused` resumes and immediately flushes the recorder before stopping. Discard, errors, and unmount all clean up tracks, queues, timers, subscriptions, and abort controllers without uploading a final chunk.
