...
- Hero motion runs on page entry. As visitors scroll, major section headings and content groups use a one-time 14px opacity/transform reveal, with short 70ms staggers for feature steps and pricing cards. The desktop trust assurances instead continuously track scroll position from left to right and resolve before the viewport center; after a short scroll-distance pause, the testimonials heading follows their final message through the same handoff. Motion is disabled by `prefers-reduced-motion` and falls back to immediately visible content if `IntersectionObserver` is unavailable.
