...
- Hero motion runs on page entry. As visitors scroll, major section headings and content groups use a one-time 14px opacity/transform reveal, with short 70ms staggers for feature steps and pricing cards. The standard landing page's desktop trust assurances continuously track scroll position from left to right and resolve before the viewport center; after a short scroll-distance pause, the testimonials heading follows their final message through the same handoff. The `/demo` variant replaces that assurance strip with a static blue HIPAA trust signal. Motion is disabled by `prefers-reduced-motion` and falls back to immediately visible content if `IntersectionObserver` is unavailable.
- Mobile navigation uses an accessible menu at 880 px and below. Features collapse to one column, while pricing becomes one compact comparison card with three plan rows and a shared unlimited-devices note. On the `/demo` route at 600 px and below, the scheduler is visually first and its supporting copy is hidden so a phone visitor reaches the booking embed immediately.
