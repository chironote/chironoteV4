...
- `WorkflowDemo.jsx` and `WorkflowDemo.css` own the shared three-step GIF carousel in the features section. It shows recording, transcript preparation, and copying the finished note into an EHR without simulating product actions on the marketing page.
...
- The `/demo` variant omits pricing and the overview-video section. Its primary “Schedule a walkthrough” action scrolls to the embedded Zoom scheduler, while the adjacent tracked “Try It Now” action opens `/app?initialState=signUp`. The scheduler uses the “LET'S CHAT” eyebrow and “Have a Human explain it” heading. The original headline/photo/layout remain; supporting copy addresses evening charting, and demo-only workflow/FAQ/calendar copy explains a guided start with the existing EHR. Both variants retain the interactive workflow demo. The top “How it works” navigation targets that workflow demo. The standard `/` copy and overview-video section remain unchanged.
...
- Testimonial avatars and workflow GIFs lazy-load.
...
`src/assets/workflow-{record,review,editor}.gif` are short current-app recordings captured on 2026-09-17 with the history drawer closed and a synthetic fictional lower-back-stiffness exam. They respectively show New Note → Exam Layout → Start Recording, Stop Recording → transcript preparation, and Subjective-section copy → full-note copy. Keep patient histories and identifiers out of future captures. Each unobstructed GIF is paired with a centered title and a short Georgia-serif explanation. The carousel links directly to ChiroNote's verified Apple App Store and Google Play listings; keep those destinations current if either listing changes.
