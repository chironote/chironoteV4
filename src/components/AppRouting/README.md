...
When `App` detects a native Capacitor runtime, `AppRoutes` exposes only `/app/*` and redirects every other path to `/app`. The shared marketing files remain in the repository for production alignment, but the wrapper does not activate public routes, route tracking, cookie consent, or advertising analytics.
