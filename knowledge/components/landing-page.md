...
- The demo variant omits pricing. Its hero pairs “Schedule a walkthrough” with a consent-aware, `landing_cta_click`-tracked “Try It Now” sign-up link to `/app?initialState=signUp`; later walkthrough actions continue to target the scheduler. The headline, clinical photo, and hero layout are preserved. Supporting copy addresses evening charting; demo-specific workflow, FAQ, and calendar copy explain getting started with Nikita over a 15-minute Zoom screen share, registration help, a mock appointment, note review/copying, and questions. The existing HIPAA FAQ explains service-provider BAAs without offering clinic-facing BAAs.
- `DemoSampleNote.jsx` adds a readable, explicitly fictional neck and right shoulder follow-up SOAP note after the video on `/demo` only. It has no patient data or captured product output. Its card stacks below the introduction at 880 px. The mobile scheduler retains the agenda and detail chips; only its eyebrow hides below 600 px.
