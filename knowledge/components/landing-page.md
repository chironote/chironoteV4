...
- The demo variant omits pricing. Its hero pairs “Schedule a walkthrough” with a consent-aware, `landing_cta_click`-tracked “Try It Now” sign-up link to `/app?initialState=signUp`; later walkthrough actions continue to target the scheduler. Its overview video has only the shared “Watch our explainer video” heading, and the scheduler introduces itself as “LET'S CHAT” and “Have a Human explain it,” then moves directly to its embed. The headline, clinical photo, and hero layout are preserved. Supporting copy addresses evening charting; demo-specific workflow, FAQ, and calendar copy explains getting started with Nikita over a 15-minute Zoom screen share, registration help, a mock appointment, note review/copying, and questions. The top “How it works” navigation targets the three-step workflow demo. The existing HIPAA FAQ explains service-provider BAAs without offering clinic-facing BAAs.
...
Three screenshots were captured from the current app on 2026-09-17 using only the user-provided sacral-pain sample with history hidden. Compressed WebP screenshots lazy-load and remain unobstructed; centered Georgia-serif copy supplies the small amount of context needed by a cold prospect. The section is headed “Use it in 3 simple steps.” The first desktop step is centered without a numbered rail; after advancing, a compact larger numbered selector appears to the left while the previous/next controls stay outside the card edges. Compact layouts hide the numbered rail, retain the three-dot state indicator, and support horizontal swipes with a visible hint. At phone width, all three screenshot-and-lesson panels share a horizontally translated track so swipe, arrow, and autoplay changes slide smoothly with the same spatial continuity as the testimonial carousel. The carousel advances every seven seconds, pauses during hover or keyboard interaction, and includes an explicit pause control; reduced-motion disables automatic advancement and transitions. Its compact store badges link to the verified ChiroNote listings on Apple's App Store (`id6756679857`) and Google Play (`com.chironote.app`). Marketing copy intentionally says SOAP even though the authenticated product exposes separate SOAP and Treatment controls.
