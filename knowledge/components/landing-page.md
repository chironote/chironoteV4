...
- The demo variant omits pricing and the overview-video section. Its hero pairs “Schedule a walkthrough” with a consent-aware, `landing_cta_click`-tracked “Try It Now” sign-up link to `/app?initialState=signUp`; later walkthrough actions continue to target the scheduler. The scheduler introduces itself as “LET'S CHAT” and “Have a Human explain it,” then moves directly to its embed. The headline, clinical photo, and hero layout are preserved. Supporting copy addresses evening charting; demo-specific workflow, FAQ, and calendar copy explains getting started with Nikita over a 15-minute Zoom screen share, registration help, a mock appointment, note review/copying, and questions. The top “How it works” navigation targets the retained three-step workflow demo. The canonical landing page keeps its overview video. The existing HIPAA FAQ explains service-provider BAAs without offering clinic-facing BAAs.
- Proposed external Zoom event/profile copy lives in [`docs/demo-booking-copy.md`](../../docs/demo-booking-copy.md). Local page changes do not update Zoom; preserve the `chironote-demo` event URL when applying the proposed “ChiroNote Walkthrough” title.
...
Both `/` and `/demo` use `WorkflowDemo` in `#features`, replacing the static laptop image and list while preserving the variant headings. The focused carousel teaches three real steps: start a recording and speak naturally, review the generated SOAP note, then copy the full note or one section into the existing EHR. It does not imitate recording, editing, clipboard, backend generation, or EHR behavior on the marketing page.

Three short GIFs were captured from the current app on 2026-09-17 with history hidden and only a synthetic fictional lower-back-stiffness exam. They show New Note → Exam Layout → Start Recording, Stop Recording → transcript preparation, and Subjective-section copy → full-note copy. The optimized GIFs lazy-load and remain unobstructed; centered Georgia-serif copy supplies the small amount of context needed by a cold prospect. The section is headed “Use it in 3 simple steps.” The desktop media frame preserves each GIF's native 3:2 composition rather than cropping its lower edge. The first desktop step is centered without a numbered rail; after advancing, a compact larger numbered selector appears to the left while the previous/next controls stay outside the card edges. Compact layouts hide the numbered rail, retain the three-dot state indicator, and support horizontal swipes with a visible hint. At phone width, all three GIF-and-lesson panels share a horizontally translated track so swipe, arrow, and autoplay changes slide smoothly with the same spatial continuity as the testimonial carousel. The carousel starts advancing every seven seconds only after at least 35% of the component enters the viewport, and pauses again while offscreen, during hover, or during keyboard interaction. It includes an explicit pause control; reduced-motion disables automatic advancement and transitions. Its compact store badges link to the verified ChiroNote listings on Apple's App Store (`id6756679857`) and Google Play (`com.chironote.app`). Marketing copy intentionally says SOAP even though the authenticated product exposes separate SOAP and Treatment controls.
