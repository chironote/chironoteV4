...
| Sync the Android project | `npx cap sync android` | Copies the web build and regenerates Android Capacitor plugin declarations without changing the deferred iOS project. |
| Build Android debug app | `cd android; .\gradlew.bat assembleDebug` | Verifies the local Android wrapper without publishing. |
| Open Android Studio | `npm run cap:open:android` | Use after a successful build and sync when device testing is needed. |
...
- Keep Android keystores, signing properties, and local SDK paths outside Git.
...
See [Capacitor Android Application](../architecture/capacitor-android.md) before changing native dependencies, lifecycle behavior, application identity, or release configuration.
