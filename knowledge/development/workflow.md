...
Commit `62f4ec8` removed the former recording test harness. The 2026-07-15 safety pass began rebuilding focused coverage for media metadata, emitted-chunk ordering, session cancellation, late upload callbacks, SQS failure, and unmount cleanup. The active suite now has 80 tests across 13 files, including the PHI-safe telemetry, delivery, resolver, signal-health, and backend-correlation contracts. It still does not validate real browser chunk rotation, a deployed AppSync subscription migration, the complete Lambda stream, or native builds, so use the recording test matrix rather than treating a green Jest run as complete release evidence.
