...
- On compact landing layouts, the hero stacks, the clinical image uses a filled portrait/shallower tablet crop, and the three short assurances sit inside the image. On the canonical landing page desktop, the image is unobstructed and the separate assurance strip remains visible; the demo variant uses a blue HIPAA trust panel instead.
- The current working-tree landing update makes the canonical page's desktop assurances track scroll progress left-to-right, pauses briefly after `No setup required`, and then reveals the testimonials heading. The demo's HIPAA panel is static apart from the shared reduced-motion-aware reveal. This is marketing-only motion and must not become a clinical-workspace pattern.
